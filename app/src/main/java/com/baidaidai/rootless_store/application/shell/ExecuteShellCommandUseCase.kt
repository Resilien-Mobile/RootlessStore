...
class ExecuteShellCommandUseCase @Inject constructor(
    ...
    operator fun invoke(shellCommandContainer: ShellCommandContainer): Flow<ShellResult> {
        return when(shellCommandContainer.shellEnvironment){
            ShellEnvironment.AppShell -> executeShellGatewayImpl.executeCommandByAppShell(shellCommandContainer.commandContent)
            ShellEnvironment.AdbShell -> executeShellGatewayImpl.executeCommandByAdbShell(shellCommandContainer.commandContent)
            ShellEnvironment.RootShell -> executeShellGatewayImpl.executeCommandByRootShell(shellCommandContainer.commandContent)
        }
    }
}